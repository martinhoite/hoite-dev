...
const TOKEN_SCROLL_TARGET_SELECTOR = '.docs-token-block[id], .docs-token-subblock[id]';
...
function resolveTokenScrollTarget(targetId: string) {
  ...
  if (target.matches(TOKEN_SCROLL_TARGET_SELECTOR)) {
    return target;
  }

  if (target instanceof HTMLHeadingElement) {
    const tokenContainer = target.closest<HTMLElement>(TOKEN_SCROLL_TARGET_SELECTOR);
...
function scrollToTokenAnchor(hash: string) {
  if (!hash) {
    return undefined;
  }
  ...
  const tryScroll = (attempt: number) => {
    ...
    const target = resolveTokenScrollTarget(targetId);
...
export function TokenAnchorScroll() {
  useEffect(() => {
    let cleanup = scrollToTokenAnchor(window.location.hash);

    const handleHashChange = () => {
      cleanup?.();
      cleanup = scrollToTokenAnchor(window.location.hash);
    };
